...
import { listAdminAttributions, listAdminRedemptions } from '../services/api';
import type { AdminAttribution, AdminRedemption } from '../types/types';

const PAGE_SIZE = 20;
type Ledger = 'attributions' | 'redemptions';
...
export default function AdminLedgers() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok && res.data) {
        ...
        else setRedemptions(res.data.items as AdminRedemption[]);
  ...
  const rows = ledger === 'attributions' ? attributions : redemptions;
